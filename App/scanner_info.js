...
export default class scanner_info extends React.Component {
    ...
    _proTime(j){
      ...
      _finish(){
        timed[this.state.index] = this.state.date.getFullYear()+'-'+this.Gdate((this.state.date.getMonth()+1))+'-'+this.Gdate(this.state.date.getDate())+' '+this.Gdate(this.state.date.getHours())+':'+this.Gdate(this.state.date.getMinutes());
   ...
   _sub(){
     ...
     if(this.state.Ckshow && this.state.ckname == '请选择仓库'){
       Toast.showShortBottom("请选择仓库")
       return false;
     }else if(this.trim(this.state.textaera) == ''){
       Toast.showShortBottom("请填写备注")
       return false;
     }else if(flogs){
       Toast.showShortBottom("入库数量不能为空")
       return false;
     }else if(flog){
       Toast.showShortBottom("请选择生产日期")
       return false;
     }else{
       ...
       if(result.status == 1){
         ...
         Toast.showShortBottom("入库成功")
       }else{
         Toast.showShortBottom("入库失败")
       }
      })
      .catch((error) => {
        console.log(error);
                        ...
                        <Modal
                         ...
                         >
                          ...
                          <Picker
                           ...
                           selectedValue={this.state.ckDataName[0]}
