    ...
    _yes(){
      ...
      fetch('http://bcr2.intsig.net/BCRService/BCR_VCF2?user=liuhui219@126.com&pass=SJL4RB3EF5M97XXF&json=1&lang=524287', {
       ...
       .then(function (result) {
        console.log(result);
        that.setState({sb:false,data:result,cname:result.organization[0].item.name,contact:result.formatted_name[0].item,post:result.title[0].item,mobile:result.telephone[0].item.number,phone:result.telephone[1].item.number,email:result.email[0].item,address:result.label[0].item.address})
        ...
        (buttonIndex) => {
          if(buttonIndex == 0){
            that.setState({modalshowP:true,})
          }
          if(buttonIndex == 1){
             that.setState({modalshow:true,showsx:true,});
             that.getXS();
          }else{
            console.log(1)
          }
                  ...
                  <Picker
                   ...
                   <Picker
                    ...
                    <Picker
                     ...
                     pickerBtnText='确定'
                     pickerCancelBtnText='取消'
                     selectedValue={this.state.XSnameData[0]}
